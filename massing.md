...
Take a certain function. We first calculate if there is enough area already by checking the amount of points that already have that certain funcion. These points are called the children. Then we can check all the possible neighbours by making a set of all the neighbours of all the children. We scaled the score of the neighbours that differ in Y direction down by a small bit, this is to avoid excessive vertical growth. By then sorting that set based on the score for that certain function we pick the best one. This is the voxel that grows. Repeat for all the other functions.
